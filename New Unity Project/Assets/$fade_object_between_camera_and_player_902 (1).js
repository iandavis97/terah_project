   ...
   for (var offset in offsets)
   {
      ...
      for (var hit : RaycastHit in hits)
      {
         ...
         if (info == null)
         {
            ...
            {
               var mat = new Material (Shader.Find("Alpha/Diffuse"));
               mat.mainTexture = info.originalMaterials[i].mainTexture;   
               mat.color = info.originalMaterials[i].color;
               mat.color.a = 1.0;
               info.alphaMaterials[i] = mat;
            }
   ...
   for (i=0;i<fadedOutObjects.Count;i++)
   {
      ...
      else
      {
         ...
         if (totallyFadedIn == fade.alphaMaterials.length)
         {
            ...
            for (mat in fade.alphaMaterials)
               Destroy(mat);
