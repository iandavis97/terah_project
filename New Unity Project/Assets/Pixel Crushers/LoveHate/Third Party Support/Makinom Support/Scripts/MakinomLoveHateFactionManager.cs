...
namespace PixelCrushers.LoveHate.MakinomSupport
{
	...
	/// <summary>
	/// This subclass of FactionManager integrates with Makinom's save system.
	/// In a Makinom project, use it instead of FactionManager.
	/// </summary>
	[AddComponentMenu("Love\u2215Hate/Third Party/Makinom/Makinom Love Hate Faction Manager")]
	public class MakinomLoveHateFactionManager : FactionManager, ISaveData
	{

		private const string DataTag = "MakinomLoveHateFactionManager";
		...
		private void RegisterSaveData()
		{
			...
			Maki.SaveGame.RegisterCustomData(DataTag, this, false);
		}
		
		private void UnregisterSaveData() {
			if (!m_registeredSaveData) return;
			m_registeredSaveData = false;
			Maki.SaveGame.UnregisterCustomData(DataTag, false);
		}
		
		public DataObject SaveGame() {
			if (debug)
			{
				Debug.Log("Love/Hate: Saving data to Makinom save system", this);
			}
		...
		public void LoadGame(DataObject data) {
			if (data == null)
			{
				Debug.LogError("Love/Hate: Can't load data from Makinom save system; data is null");
				return;
			}
			else if (debug)
			{
				Debug.Log("Love/Hate: Loading data from Makinom save system", this);
			}
