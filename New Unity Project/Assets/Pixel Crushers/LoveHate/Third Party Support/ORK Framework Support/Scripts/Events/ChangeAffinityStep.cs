...
namespace ORKFramework.Events.Steps
{
	...
	[ORKEditorHelp("Change Affinity", "Changes the affinity of a faction for another faction.", "")]
	[ORKEventStep(typeof(BaseEvent))]
	[ORKNodeInfo("Love\u2215Hate Steps")]
	public class ChangeAffinityStep : BaseEventStep
	{
		...
		[ORKEditorHelp("Operator", "Select how the affinity value will be changed:\n" +
		...
		public override void Execute(BaseEvent baseEvent)
		{
			...
			{
				Debug.LogWarning("Love/Hate: ChangeAffinityStep - Can't find faction manager");
			}
			if (judgeID == -1)
			{
				Debug.LogWarning("Love/Hate: ChangeAffinityStep - Can't find judge faction ID");
			}
			else if (subjectID == -1)
			{
				Debug.LogWarning("Love/Hate: ChangeAffinityStep - Can't find subject faction ID");
			}
			else
			{
				switch (op)
				...
				case SimpleOperator.Set:
					factionManager.SetPersonalAffinity(judgeID, subjectID, value.GetValue(baseEvent));
					break;
				case SimpleOperator.Add:
					factionManager.ModifyPersonalAffinity(judgeID, subjectID, value.GetValue(baseEvent));
					break;
