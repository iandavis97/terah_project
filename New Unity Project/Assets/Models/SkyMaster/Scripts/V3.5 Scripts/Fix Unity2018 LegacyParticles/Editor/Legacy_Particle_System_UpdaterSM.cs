...
public class LegacyParticleUpdaterSM : ScriptableWizard
{

    public bool onlyScenes = false;//v4.2a
    ...
    public LegacyCleanupMode cleanupMode = LegacyCleanupMode.DeleteLegacyComponents;
    ...
    {
        ScriptableWizard.DisplayWizard<LegacyParticleUpdaterSM>("Upgrade Legacy Particles v" + kVersion, "Upgrade Selected", "Upgrade Everything");
    }
    ...
    void OnWizardOtherButton()
    {
        ...
        if (!onlyScenes)
        {
            ...
            //v4.2
            string[] prefabGUIDs1 = AssetDatabase.FindAssets("t:Prefab");
            //string[] prefabGUIDs = AssetDatabase.FindAssets("t:BoxCollider", new[] { "Assets" });
            Debug.Log("Prefabs Searched:" + prefabGUIDs1.Length);
            List<ParticleEmitter> emitterComponents = new List<ParticleEmitter>();
            for (int i = 0; i < prefabGUIDs1.Length; i++)
            {
                ParticleEmitter[] emitter = AssetDatabase.LoadAssetAtPath<GameObject>(AssetDatabase.GUIDToAssetPath(prefabGUIDs1[i])).GetComponentsInChildren<ParticleEmitter>(true);
                for (int i1 = 0; i1 < emitter.Length; i1++)//if (emitter != null)
                {
                    emitterComponents.Add(emitter[i1]);
                }
                //prefabs[i] = //AssetDatabase.LoadAssetAtPath<GameObject>(AssetDatabase.GUIDToAssetPath(prefabGUIDs1[i])).GetComponent<ParticleEmitter>();
            }

            prefabs = new ParticleEmitter[emitterComponents.Count];
            for (int i = 0; i < emitterComponents.Count; i++)
            {
                prefabs[i] = emitterComponents[i];
            }
            Debug.Log("Prefabs with Legacy Particles found:" + prefabs.Length);
            //return;
        }

        //string[] prefabGUIDs = AssetDatabase.FindAssets("t:ParticleEmitter");
        ////string[] prefabGUIDs = AssetDatabase.FindAssets("t:BoxCollider", new[] { "Assets" });
        //Debug.Log(prefabGUIDs.Length);
    ...
    void Upgrade(ParticleEmitter emitter)
    {
        ...
        //NASOS v4.2
        //if (emitter.GetComponent<ChainLightningShuriken_SM>() != null)
        if (emitter.GetComponent<ChainLightning_SKYMASTER>() != null)
        {
            emitter.GetComponent<ChainLightning_SKYMASTER>().line_on = true;
            emitter.GetComponent<ChainLightning_SKYMASTER>().line_glow_on = true;
            emitter.GetComponent<ChainLightning_SKYMASTER>().Branches = true;

            //if chain lightining, disable particle renderer
            //emitter.GetComponent<ParticleSystem>().enableEmission = false;
            ParticleSystem.EmissionModule emitterModule = emitter.GetComponent<ParticleSystem>().emission;
            emitterModule.enabled = false;            

            //emitter.GetComponent<ParticleSystem>().renderer.
            if (emitter.GetComponent<LineRenderer>() == null)
            {
                Undo.AddComponent<LineRenderer>(emitter.gameObject);
                emitter.GetComponent<LineRenderer>().material = lightning_material;
            }
        }
        ...
        {
            if (emitter.gameObject.GetComponent<ParticleAnimator>() != null)
            {
                Undo.DestroyObjectImmediate(emitter.gameObject.GetComponent<ParticleAnimator>());
            }
            if (emitter.gameObject.GetComponent<ParticleRenderer>() != null)
            {
