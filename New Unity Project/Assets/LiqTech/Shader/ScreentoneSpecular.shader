Shader "Screentone/Specular" {
        Properties {
			...
			_GlossTexture ("Gloss Pattern", 2D) = "white" {}
			_GlossTextureSize("Gloss Texture Size", Int) = 32
			_GlossRotationAngle("Gloss Texture Rotation", Range(0, 6.3)) = 0
			_GlossPrimaryColor ("Gloss Primary Color", Color) = (1,1,1,1)
			_GlossSecondaryColor ("Gloss Secondary Color", Color) = (0,0,0,1)

			_GlossPower("Gloss power", Float) = 10
		...
		fixed4 _GlossPrimaryColor;
		fixed4 _GlossSecondaryColor;
		float _GlossRotationAngle;
		int  _GlossTextureSize;

		float _GlossBrightness;
		float _GlossPower;
		...
		half4 LightingSimpleLambert (SurfaceOutputCustom s, half3 lightDir,  half3 viewDir, half atten) {
			...
			half spec = pow (nh, _GlossPower) * _LightColor0.r;


			if(brightness + spec < _ShadedThreshold)
			{
				return half4(s.Shaded, 1);
			}
			else if(brightness + spec > 1)
			{
				return half4(s.Gloss, 1);
			}
			else
			{
        ...
        void surf (Input IN, inout SurfaceOutputCustom o) {
			...
			float2 screenUVC = IN.screenPos.xy / IN.screenPos.w;
			screenUVC.x *= _ScreenParams.x / _ScreenParams.y;
			screenUVC *= _ScreenParams.y / _GlossTextureSize;
			screenUVC = rotate(screenUVC, _GlossRotationAngle);
			#ifdef FULLCOLOR
			o.Gloss = tex2D (_GlossTexture, screenUVC) * _GlossPrimaryColor;
			#else
			float cc = tex2D (_GlossTexture, screenUVC).a;
			o.Gloss = lerp(_GlossSecondaryColor, _GlossPrimaryColor, cc);
			#endif
        }
        ENDCG
        }
        Fallback "Diffuse"
		CustomEditor "ScreentoneSpecularEditor"
    }