...
public class ScreentoneSpecularEditor : ShaderGUI
{
    override public void OnGUI(MaterialEditor materialEditor, MaterialProperty[] properties)
    {
        ...
        EditorGUILayout.LabelField(new GUIContent("Shaded texture"), EditorStyles.boldLabel);
        GUILayout.BeginHorizontal();
        var r3 = EditorGUILayout.GetControlRect(false, 128, GUILayout.Width(128));
        properties[11].textureValue = (Texture2D)EditorGUI.ObjectField(r3, properties[11].textureValue, typeof(Texture2D), false);
        GUILayout.BeginVertical();
        properties[12].floatValue = EditorGUILayout.FloatField("Size", properties[12].floatValue);
        if (GUILayout.Button("Set native size")) properties[12].floatValue = properties[11].textureValue == null ? 0 : properties[11].textureValue.width;
        properties[13].floatValue = Mathf.Deg2Rad * EditorGUILayout.Slider("Angle", properties[13].floatValue * Mathf.Rad2Deg, 0, 360);

        properties[14].colorValue = EditorGUILayout.ColorField("Primary color", properties[14].colorValue);
        if (isTwoTone) properties[15].colorValue = EditorGUILayout.ColorField("Secondary color", properties[15].colorValue);
        GUILayout.Space(18);
        properties[16].floatValue = EditorGUILayout.Slider("Gloss power", properties[16].floatValue, 1, 100);
        GUILayout.EndVertical();
        GUILayout.EndHorizontal();
        ...
        var isTwoSided = properties[18].floatValue == 0;
        var wantTwoSided = EditorGUILayout.Toggle("Double sided", isTwoSided);
        properties[18].floatValue = wantTwoSided ? 0 : 2;
    }
}
